...
const AddCustomerSalePage: React.FC<AddCustomerSalePageProps> = () => {
  ...
  const productOptions = useMemo(() => {
    ...
    dealerProducts.forEach((stock) => {
      if (!productsMap.has(stock.product.id.toString())) {
        productsMap.set(stock.product.id.toString(), stock.product.name);
      }
    });

    return Array.from(productsMap.entries()).map(([value, label]) => ({
      value,
      label,
    }));
  }, [dealerProducts]);
  ...
  const availableBatches = useMemo(
    () =>
      dealerProducts.filter(
        (stock) =>
          stock.product.id.toString() === selectedProductId &&
          stock.status === "ACTIVE" &&
          stock.quantity > 0
      ),
    [dealerProducts, selectedProductId]
  );
  ...
  const createSaleMutation = useMutation({
    mutationFn: async (data: AddCustomerSaleForm) => {
      ...
      const selectedStock = await searchDealerStockApi(
        dealerId,
        productId,
        data.batch_number
      );
      ...
      if (!selectedStock) {
        throw new Error("Selected batch was not found in dealer stock");
      }
      ...
      await updateDealerStockApi(selectedStock.id, selectedStock.quantity - 1);
  ...
  const selectedBatchPreview = dealerProducts.find(
    (stock) =>
      stock.product.id.toString() === pendingFormData?.product_id &&
      stock.batch_number === pendingFormData?.batch_number
  );
                ...
                <Descriptions column={1} size="middle" bordered>
                  ...
                  <Descriptions.Item label="Stock Impact">
                    <div className="flex items-center gap-2">
                      <Tag color="blue">
                        Current: {selectedBatchPreview?.quantity || 0} units
                      </Tag>
                      <span>→</span>
                      <Tag color="orange">
                        After Sale: {(selectedBatchPreview?.quantity || 0) - 1} units
                      </Tag>
                    </div>
                  </Descriptions.Item>
          ...
          <Card title="Customer Sale Information" className="shadow-sm">
            <FormProvider {...methods}>
              ...
              >
                      ...
                      <div className="mt-4">
                        <MultiSelect<AddCustomerSaleForm>
                          ...
                          options={availableBatches.map((stock) => ({
                            label: `${stock.batch_number} - Stock: ${stock.quantity}`,
                      ...
                      <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-blue-700 text-sm mt-1">
                              Select a product first, then choose a batch with
                              available stock. Each sale reduces selected batch
                              stock by 1 unit.
                            </p>
                          </div>
                        </div>
                ...
                {/* Information Note */}
                <div className="bg-linear-to-r from-orange-50 to-red-50 rounded-lg p-6 border border-orange-200">
                  <div className="flex items-start gap-3">
                    <svg
                      className="w-6 h-6 text-orange-500 mt-0.5 shrink-0"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                    ...
                    <div>
                      ...
                      <ul className="text-xs text-gray-600 space-y-1 list-disc list-inside">
                        ...
                        <li>
                          Selected batch stock quantity will be reduced by 1
                          unit automatically
                        </li>
                        <li>
