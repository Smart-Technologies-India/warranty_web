...
const fetchSaleById = async (
  ...
  const response = await ApiCall<{ getSalesById: SaleDetails }>({
  ...
  const sale = response.data.getSalesById;

  if (!sale || sale.dealer.id !== dealerId) {
    return null;
  }
...
const SaleDetailsPage: React.FC<SaleDetailsPageProps> = () => {
  ...
  const dealerId: number = parseInt(params.id as string);
  const saleId: number = parseInt(params.saleId as string);
