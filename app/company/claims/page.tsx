...
const CustomerClaimsPage: React.FC = () => {
  ...
  const currentUserId: number = parseInt(getCookie("id")?.toString() || "0");
  const role: string = String(getCookie("role") || "");
  const companyId: number = parseInt(getCookie("company")?.toString() || "0"); // You can get this from authentication or route params
  const isPrivilegedRole = ["MANUF_ADMIN", "MANUF_MANAGER"].includes(role);
  ...
  const backendTake = isPrivilegedRole ? pageSize : 1000;
  const backendSkip = isPrivilegedRole ? skip : 0;
  ...
  const {
    ...
    queryKey: ["companyTickets", companyId, role, backendSkip, backendTake],
    queryFn: () => fetchTickets(companyId, backendTake, backendSkip),
    enabled: !!companyId,
  });

  const accessFilteredTickets =
    ticketsData?.data?.filter((ticket) => {
      if (isPrivilegedRole) return true;

      if (!ticket.ticket_levels || ticket.ticket_levels.length === 0) return false;

      const latestLevel = ticket.ticket_levels
        .slice()
        .sort((a, b) => b.level - a.level || b.id - a.id)[0];

      return latestLevel?.assigned_to_id === currentUserId;
    }) || [];

  // Filter tickets based on search term
  const filteredTickets =
    accessFilteredTickets.filter(
      (ticket) =>
  ...
  const paginatedTickets = isPrivilegedRole
    ? filteredTickets
    : filteredTickets.slice(skip, skip + pageSize);

  const totalVisibleTickets = isPrivilegedRole
    ? ticketsData?.total || 0
          ...
          <Row gutter={16} align="middle">
            ...
            <Col xs={24} md={12}>
              <div className="flex justify-end items-center gap-4 mt-4 md:mt-0">
                <Text className="text-gray-600">
                  Total: {totalVisibleTickets} tickets
                </Text>
              </div>
            </Col>
          </Row>
          ...
          <Col xs={12} md={6}>
            <Card size="small" className="text-center">
              <div className="text-2xl font-bold text-blue-600">
                {accessFilteredTickets.filter((t) => t.status === "OPEN").length || 0}
              </div>
          ...
          <Col xs={12} md={6}>
            <Card size="small" className="text-center">
              <div className="text-2xl font-bold text-orange-600">
                {accessFilteredTickets.filter((t) => t.status === "IN_PROGRESS").length || 0}
              </div>
          ...
          <Col xs={12} md={6}>
            <Card size="small" className="text-center">
              <div className="text-2xl font-bold text-green-600">
                {accessFilteredTickets.filter((t) => t.status === "RESOLVED").length ||
                  0}
              </div>
