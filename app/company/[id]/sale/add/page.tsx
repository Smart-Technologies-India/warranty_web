...
const AddSalePage: React.FC<AddSalePageProps> = () => {
  ...
  const { data: manfStocks = [], isLoading: isManfStocksLoading } = useQuery({
  ...
  const createSaleMutation = useMutation({
    mutationFn: async (data: AddSaleForm) => {
      ...
      const saleData: CreateSaleInput = {
        ...
        dealer_purch_id: parseInt(data.dealer_id),
        dealer_seller_id: companyId,
      ...
      if (manfStock.quantity < data.quantity) {
        throw new Error(
          `Only ${manfStock.quantity} units available in selected batch`
        );
      }
      ...
      // Reduce manufacturer stock for sold quantity
      await updateManfStockApi(
        manfStock.id,
        manfStock.quantity - data.quantity,
